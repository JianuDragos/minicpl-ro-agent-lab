...
class DualAgentArena:
    def __init__(
        ...
        self.phase_name = DEBATE_PHASE_TEMPLATE.format(phase_id=phase_id)
        ...
        self.protocol = self._load_latest_protocol_state()
    ...
    def run(self) -> dict[str, Any]:
        ...
        jsonl_path = results_dir / f"phase{self.phase_id}_debate_{run_id}.jsonl"
        md_path = results_dir / f"phase{self.phase_id}_debate_{run_id}.md"
        state_path = results_dir / f"protocol_state_phase{self.phase_id}_{run_id}.json"
        ...
        with jsonl_path.open("w", encoding="utf-8") as jsonl:
            ...
            for turn in range(1, self.debate_turns_per_agent + 1):
                ...
                test_result = self._run_sender_receiver_test(turn) if self.receiver_test else None
                if test_result:
    ...
    def _run_sender_receiver_test(self, turn: int) -> dict[str, Any]:
